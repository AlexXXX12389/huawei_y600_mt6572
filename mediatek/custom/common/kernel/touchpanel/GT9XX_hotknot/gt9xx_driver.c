...
static s32 gtp_init_panel(struct i2c_client *client)
{
    ...
    if (ret == SUCCESS){
        GTP_DEBUG("CFG_CONFIG_GROUP%d Config Version: %d, 0x%02X; IC Config Version: %d, 0x%02X", sensor_id+1, send_cfg_buf[sensor_id][0], send_cfg_buf[sensor_id][0], opr_buf[0], opr_buf[0]);
        if (opr_buf[0] < 90){
...
static int tpd_registration(void *client)
{
		...
		ret = tpd_power_on((struct i2c_client *)client);
	...
	//#ifdef VELOCITY_CUSTOM
		...
		ret = gtp_read_version((struct i2c_client *)client, &version_info);
		...
		ret = gtp_init_panel((struct i2c_client *)client);
...
#if GTP_CREATE_WR_NODE
		init_wr_node((struct i2c_client *)client);
#endif
...
#if GTP_AUTO_UPDATE
		ret = gup_init_update_proc((struct i2c_client *)client);
...
#if GTP_ESD_PROTECT
		gtp_esd_switch((struct i2c_client *)client, SWITCH_ON);
#endif
